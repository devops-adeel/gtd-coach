#!/usr/bin/env python3
"""
Upload ONLY the missing GTD Coach prompts to Langfuse.
Skips gtd-coach-system and gtd-coach-fallback which already exist.
"""
...
def upload_missing_prompts():
    """Upload only the missing prompts to Langfuse"""
    
    # Initialize Langfuse client (uses env vars)
    print("Initializing Langfuse client...")
    langfuse = Langfuse()
    
    # Read prompt files for reference
    prompts_dir = Path.home() / "gtd-coach" / "config" / "prompts"
    
    print("\nSkipping existing prompts: gtd-coach-system, gtd-coach-fallback")
    print("Uploading only missing prompts...")
    
    # =====================================================
    # Upload only NEW prompts
    # =====================================================
    
    # Firm tone prompt (as separate prompt)
...
Score (0.0-1.0): """
    ...
    print("\n✅ All missing prompts uploaded successfully!")
    print(f"Total new prompts uploaded: 10")
    print("Existing prompts preserved: 2 (gtd-coach-system, gtd-coach-fallback)")
    print("\nTotal prompts now in Langfuse: 12")

if __name__ == "__main__":
    upload_missing_prompts()